    ...
    def move(self, piece, move, testing=False):
        ...
        if isinstance(piece, Pawn):
            ...
            if diff != 0 and en_passant_empty:
                ...
                self.squares[final.row][final.col].piece = piece
                if not testing:
                    sound = Sound(os.path.join(r'C:\Users\Rohan\Downloads\python-chess-ai-yt-master\python-chess-ai-yt-master\assets\sounds\capture.wav'))
                    sound.play()
            else:
        ...
        def king_moves():
            ...
            for r_offset, c_offset in directions:
                ...
                if Square.in_range(r, c):
                    if self.squares[r][c].isempty_or_enemy(piece.color):
                        initial = Square(row, col)
                        final = Square(r, c, self.squares[r][c].piece)
                        move = Move(initial, final)
                        if bool:
                            if not self.in_check(piece, move):
                                piece.add_move(move)
